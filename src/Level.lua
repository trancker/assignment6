...
function Level:update(dt)
    ...
    if self.hit then
        self.launchMarker.collides = true
    end
    ...
    self.players = {}
    table.insert(self.players, self.launchMarker.alien)
    table.insert(self.players, self.launchMarker.alien1)
    table.insert(self.players, self.launchMarker.alien2)
    ...
    if self.launchMarker.launched then
        ...
        for i = #self.players, 1, -1 do
            local xPos, yPos = self.players[i].body:getPosition()
            local xVel, yVel = self.players[i].body:getLinearVelocity()

            -- if we fired our alien to the left or it's almost done rolling, respawn
            if xPos < 0 or (math.abs(xVel) + math.abs(yVel) < 1.5) then
                self.hit = false
                table.insert(self.dest, self.players[i])
                table.remove(self.players, i)

                if restart > #self.players then
                    for j = #self.dest, 1, -1 do
                        self.dest[j].body:destroy()
                    end
                    self.launchMarker = AlienLaunchMarker(self.world)
