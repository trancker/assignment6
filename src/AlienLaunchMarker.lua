...
function AlienLaunchMarker:update(dt)
    ...
    if not self.launched then
        ...
        if love.keyboard.wasPressed('space') and not self.collides and self.count == 1 then
            for i = 1, 2 do
                self.alien1 = Alien(self.world, 'round', self.alien.body:getX(), self.alien.body:getY() + 30, 'Player')
                self.alien1.body:setLinearVelocity((self.baseX - self.shiftedX) * 10, (self.baseY - self.shiftedY) * 10 + 10)
                self.alien1.fixture:setRestitution(0.4)
                self.alien1.body:setAngularDamping(1)

                self.alien2 = Alien(self.world, 'round', self.alien.body:getX(), self.alien.body:getY() - 30, 'Player')
                self.alien2.body:setLinearVelocity((self.baseX - self.shiftedX) * 10, (self.baseY - self.shiftedY) * 10 - 30)
